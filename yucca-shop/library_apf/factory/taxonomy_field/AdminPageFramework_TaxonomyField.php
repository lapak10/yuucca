...
abstract class yucca_shopAdminPageFramework_TaxonomyField_Router extends yucca_shopAdminPageFramework_Factory
{
    public function __construct($oProp)
    {
        ...
        $this->oUtil->registerAction('wp_loaded', [$this, '_replyToDetermineToLoad']);
    ...
    public function _replyToSetUpHooks($oFactory)
    {
        foreach ($this->oProp->aTaxonomySlugs as $_sTaxonomySlug) {
            add_action("created_{$_sTaxonomySlug}", [$this, '_replyToValidateOptions'], 10, 2);
            add_action("edited_{$_sTaxonomySlug}", [$this, '_replyToValidateOptions'], 10, 2);
            add_action("{$_sTaxonomySlug}_add_form_fields", [$this, '_replyToPrintFieldsWOTableRows']);
            add_action("{$_sTaxonomySlug}_edit_form_fields", [$this, '_replyToPrintFieldsWithTableRows']);
            add_filter("manage_edit-{$_sTaxonomySlug}_columns", [$this, '_replyToManageColumns'], 10, 1);
            add_filter("manage_edit-{$_sTaxonomySlug}_sortable_columns", [$this, '_replyToSetSortableColumns']);
            add_action("manage_{$_sTaxonomySlug}_custom_column", [$this, '_replyToPrintColumnCell'], 10, 3);
        }
    }
}
abstract class yucca_shopAdminPageFramework_TaxonomyField_Model extends yucca_shopAdminPageFramework_TaxonomyField_Router
{
    ...
    protected function _setOptionArray($iTermID, $sOptionKey)
    {
    ...
    public function _replyToValidateOptions($iTermID)
    {
        ...
        $_aSubmittedFormData = $this->oUtil->addAndApplyFilters($this, 'validation_'.$this->oProp->sClassName, call_user_func_array([$this, 'validate'], [$_aSubmittedFormData, $_aSavedFormData, $this]), $_aSavedFormData, $this);
...
abstract class yucca_shopAdminPageFramework_TaxonomyField_View extends yucca_shopAdminPageFramework_TaxonomyField_Model
{
...
abstract class yucca_shopAdminPageFramework_TaxonomyField_Controller extends yucca_shopAdminPageFramework_TaxonomyField_View
{
    public function setUp()
    {
    }
}
abstract class yucca_shopAdminPageFramework_TaxonomyField extends yucca_shopAdminPageFramework_TaxonomyField_Controller
{
    protected static $_sStructureType = 'taxonomy_field';

    public function __construct($asTaxonomySlug, $sOptionKey = '', $sCapability = 'manage_options', $sTextDomain = 'yucca-shop')
    {
        if (empty($asTaxonomySlug)) {
            return;
        }
        $this->oProp = new yucca_shopAdminPageFramework_Property_taxonomy_field($this, get_class($this), $sCapability, $sTextDomain, self::$_sStructureType);
