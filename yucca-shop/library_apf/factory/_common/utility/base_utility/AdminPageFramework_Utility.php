...
abstract class yucca_shopAdminPageFramework_Utility_Deprecated
{
    ...
    public static function getAttributes(array $aAttributes)
    {
        ...
        foreach ($aAttributes as $sAttribute => $sProperty) {
            if (in_array(gettype($sProperty), ['array', 'object'])) {
                continue;
            }
...
abstract class yucca_shopAdminPageFramework_Utility_VariableType extends yucca_shopAdminPageFramework_Utility_Deprecated
{
...
abstract class yucca_shopAdminPageFramework_Utility_String extends yucca_shopAdminPageFramework_Utility_VariableType
{
    ...
    {
        trigger_error(yucca_shopAdminPageFramework_Registry::NAME.': '.sprintf('The method, %1$s, is deprecated. Use %2$s instead.', 'minifyCSS()', 'getCSSMinified()'), E_USER_NOTICE);
    ...
    public static function getReadableBytes($nBytes)
    {
        $_aUnits = [0 => 'B', 1 => 'kB', 2 => 'MB', 3 => 'GB'];
...
abstract class yucca_shopAdminPageFramework_Utility_Array extends yucca_shopAdminPageFramework_Utility_String
{
    ...
    public static function getReadableArrayContents($sKey, $vValue, $sLabelCharLengths = 16, $iOffset = 0)
    {
        ...
        if (!in_array(gettype($vValue), ['array', 'object'])) {
            $_aOutput[] = $vValue;

            return implode(PHP_EOL, $_aOutput);
        }
        foreach ($vValue as $_sTitle => $_asDescription) {
            if (!in_array(gettype($_asDescription), ['array', 'object'])) {
...
abstract class yucca_shopAdminPageFramework_Utility_ArrayGetter extends yucca_shopAdminPageFramework_Utility_Array
{
    ...
    public static function getElement($aSubject, $aisKey, $mDefault = null, $asToDefault = [null])
    {
        $_aToDefault = is_null($asToDefault) ? [null] : self::getAsArray($asToDefault, true);
    ...
    public static function getElementAsArray($aSubject, $aisKey, $mDefault = null, $asToDefault = [null])
    {
...
abstract class yucca_shopAdminPageFramework_Utility_ArraySetter extends yucca_shopAdminPageFramework_Utility_ArrayGetter
{
    ...
    public static function dropElementsByType(array $aArray, $aTypes = ['array'])
    {
...
abstract class yucca_shopAdminPageFramework_Utility_Path extends yucca_shopAdminPageFramework_Utility_ArraySetter
{
    ...
    private static $_aCallerScriptPathCaches = [];
}
abstract class yucca_shopAdminPageFramework_Utility_URL extends yucca_shopAdminPageFramework_Utility_Path
{
    public static function getQueryValueInURLByKey($sURL, $sQueryKey)
    {
        $_aURL = parse_url($sURL) + ['query' => ''];
    ...
    private static function _getURLPortSuffix($_bSSL)
    {
...
abstract class yucca_shopAdminPageFramework_Utility_File extends yucca_shopAdminPageFramework_Utility_URL
{
    public static function getFileTailContents($asPath = [], $iLines = 1)
    {
    ...
    {
        $_aItems = is_array($asItems) ? $asItems : [$asItems];
...
abstract class yucca_shopAdminPageFramework_Utility_SystemInformation extends yucca_shopAdminPageFramework_Utility_File
{
    ...
    {
        $_aCategories = is_array($asCategories) ? $asCategories : [$asCategories];
        $_aCategories = array_filter($_aCategories);
        $_aRemovingCategories = is_array($asRemovingCategories) ? $asRemovingCategories : [$asRemovingCategories];
...
abstract class yucca_shopAdminPageFramework_Utility_HTMLAttribute extends yucca_shopAdminPageFramework_Utility_SystemInformation
{
    ...
    public static function getClassAttribute()
    {
        ...
        foreach (func_get_args() as $_asClasses) {
            if (!in_array(gettype($_asClasses), ['array', 'string'])) {
                continue;
            }
    ...
    public static function generateClassAttribute()
    {
        ...
        return call_user_func_array([__CLASS__, 'getClassAttribute'], $_aParams);
    }
...
class yucca_shopAdminPageFramework_Utility extends yucca_shopAdminPageFramework_Utility_HTMLAttribute
{
    private static $_aCallStack = [];

    public static function hasBeenCalled($sID)
    {
    ...
    public static function getOutputBuffer($oCallable, array $aParameters = [])
    {
        ob_start();
