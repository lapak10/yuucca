...
class yucca_shopAdminPageFramework_TabNavigationBar extends yucca_shopAdminPageFramework_FrameworkUtility
{
    public $sTabTag = 'h2';
    public $aTabs = [];
    public $aAttributes = ['class' => 'nav-tab-wrapper'];
    public $aTab = ['slug' => null, 'title' => null, 'href' => null, 'disabled' => null, 'class' => null, 'attributes' => []];
    public $aCallbacks = ['format' => null, 'arguments' => []];

    public function __construct(array $aTabs, $asActiveTabSlugs, $sTabTag = 'h2', $aAttributes = ['class' => 'nav-tab-wrapper'], $aCallbacks = [])
    {
        $this->aCallbacks = $aCallbacks + ['format' => null, 'arguments' => null];
    ...
    {
        $aTab = is_callable($this->aCallbacks['format']) ? call_user_func_array($this->aCallbacks['format'], [$aTab, $this->aTab, $aTabs, $this->aCallbacks['arguments']]) : $aTab;
    ...
    private function _getTabs()
    {
        ...
        $_aContainerAttributes = $this->aAttributes + ['class' => null];
    ...
    {
        $_aATagAttributes = isset($aTab['attributes']) ? $aTab['attributes'] : [];
        ...
        $_aATagAttributes = ['class' => $_sClassAttribute] + $_aATagAttributes + ['href' => $aTab['href'], 'title' => $aTab['title']];
        if ($aTab['disabled']) {
