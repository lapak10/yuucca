...
abstract class yucca_shopAdminPageFramework_HelpPane_Base extends yucca_shopAdminPageFramework_FrameworkUtility
{
    public $oProp;
    protected $_oScreen;
    ...
    {
        $this->oProp = $oProp;
        add_action('admin_head', [$this, '_replyToRegisterHelpTabText']);
    }

    protected function _setHelpTab($sID, $sTitle, $aContents, $aSideBarContents = [])
    {
        if (empty($aContents)) {
            return;
        }
        $this->_oScreen = isset($this->_oScreen) ? $this->_oScreen : get_current_screen();
        $this->_oScreen->add_help_tab(['id' => $sID, 'title' => $sTitle, 'content' => implode(PHP_EOL, $aContents)]);
