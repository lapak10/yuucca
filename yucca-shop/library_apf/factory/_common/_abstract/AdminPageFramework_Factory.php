...
abstract class yucca_shopAdminPageFramework_Factory_Router
{
    public $oProp;
    public $oDebug;
    public $oUtil;
    ...
    protected $oHelpPane;
    protected $oLink;
    protected $_aSubClassNames = ['oDebug', 'oUtil', 'oMsg', 'oForm', 'oPageLoadInfo', 'oResource', 'oHelpPane', 'oLink'];
    ...
    public function __construct($oProp)
    {
        ...
        if ($this->oProp->bIsAdmin && !$this->oProp->bIsAdminAjax) {
            if (did_action('current_screen')) {
                ...
                add_action('current_screen', [$this, '_replyToLoadComponents']);
            }
        }
        $this->start();
    ...
    protected function _getFormObject($oProp)
    {
        ...
        $_sFormClass = "yucca_shopAdminPageFramework_Form_{$oProp->_sPropertyType}";
    ...
    public function __get($sPropertyName)
    {
        if (in_array($sPropertyName, $this->_aSubClassNames)) {
            return call_user_func([$this, "_replyTpSetAndGetInstance_{$sPropertyName}"]);
        }
    }

    public function _replyTpSetAndGetInstance_oUtil()
    {
        $this->oUtil = new yucca_shopAdminPageFramework_FrameworkUtility();
    ...
    {
        $this->oDebug = new yucca_shopAdminPageFramework_Debug();
    ...
    {
        $this->oMsg = yucca_shopAdminPageFramework_Message::getInstance($this->oProp->sTextDomain);
    ...
    public function _replyTpSetAndGetInstance_oResource()
    {
        ...
        $_sClassName = "yucca_shopAdminPageFramework_Resource_{$this->oProp->_sPropertyType}";
    ...
    {
        $_sClassName = "yucca_shopAdminPageFramework_HelpPane_{$this->oProp->_sPropertyType}";
    ...
    private function _getAutoCallback($sMethodName, $aArguments)
    {
        ...
        return method_exists($this, $_sAutoCallbackMethodName) ? call_user_func_array([$this, $_sAutoCallbackMethodName], $aArguments) : $this->oUtil->getElement($aArguments, 0);
    }

    private function _triggerUndefinedMethodWarning($sMethodName)
    {
        trigger_error(yucca_shopAdminPageFramework_Registry::NAME.': '.sprintf(__('The method is not defined: %1$s', $this->oProp->sTextDomain), $sMethodName), E_USER_WARNING);
    }
...
abstract class yucca_shopAdminPageFramework_Factory_Model extends yucca_shopAdminPageFramework_Factory_Router
{
    public function __construct($oProp)
    {
        parent::__construct($oProp);
        add_filter('field_types_'.$oProp->sClassName, [$this, '_replyToFilterFieldTypeDefinitions']);
    }
    ...
    {
        $aFieldset = $aFieldset + ['help' => null, 'title' => null, 'help_aside' => null];
        if (!$aFieldset['help']) {
            return;
        }
    ...
    public function _replyToFilterFieldTypeDefinitions($aFieldTypeDefinitions)
    {
        if (method_exists($this, 'field_types_'.$this->oProp->sClassName)) {
            return call_user_func_array([$this, 'field_types_'.$this->oProp->sClassName], [$aFieldTypeDefinitions]);
        }
    ...
    public function _replyToFormatSectionsetDefinition($aSectionset)
    {
        ...
        $aSectionset = $aSectionset + ['_fields_type' => $this->oProp->_sPropertyType, '_structure_type' => $this->oProp->_sPropertyType];
...
abstract class yucca_shopAdminPageFramework_Factory_View extends yucca_shopAdminPageFramework_Factory_Model
{
    public function __construct($oProp)
    {
        parent::__construct($oProp);
        new yucca_shopAdminPageFramework_Factory_View__SettingNotice($this, $this->oProp->sSettingNoticeActionHook);
    }
    ...
    {
        $_aParams = func_get_args() + [null, null, null, null];
    ...
    {
        $aElementDefinition = $aElementDefinition + ['if' => true, 'capability' => ''];
    ...
    {
        $aFieldset = $aFieldset + ['section_id' => null];
    ...
    {
        return $this->oUtil->addAndApplyFilters($this, ['section_head_'.$this->oProp->sClassName.'_'.$aSectionset['section_id']], $sSectionDescription);
    }
    ...
    public function _replyToGetFieldOutput($sFieldOutput, $aFieldset)
    {
        ...
        return $this->oUtil->addAndApplyFilters($this, ['field_'.$this->oProp->sClassName.$_sSectionPart.'_'.$aFieldset['field_id']], $sFieldOutput, $aFieldset);
    }
}
abstract class yucca_shopAdminPageFramework_Factory_Controller extends yucca_shopAdminPageFramework_Factory_View
{
    ...
    public function setSettingNotice($sMessage, $sType = 'error', $asAttributes = [], $bOverride = true)
    {
...
abstract class yucca_shopAdminPageFramework_Factory extends yucca_shopAdminPageFramework_Factory_Controller
{
}
