...
class yucca_shopAdminPageFramework_Form_View___Script_RepeatableField extends yucca_shopAdminPageFramework_Form_View___Script_Base
{
...
(function ( $ ) {
     ...
     */
    $.fn.updateyucca_shopAdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( sFieldsContainerID ) ) {     
            $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ];
        ...
        $( nodeThis ).find( '.yucca-shop-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.yucca-shop-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.yucca-shop-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.yucca-shop-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            $( this ).addyucca_shopAdminPageFrameworkRepeatableField();
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removeyucca_shopAdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });     
        
        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.yucca-shop-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + sFieldsContainerID ).find( '.yucca-shop-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addyucca_shopAdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addyucca_shopAdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.yucca-shop-field' ).attr( 'id' );    
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.yucca-shop-fields' );
        var sFieldsContainerID  = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.yucca-shop-repeatable-field-buttons' );
            $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.yucca-shop-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.yucca-shop-repeatable-field-buttons' ).last();
         ...
         * */
        nodeNewField.updateyucca_shopAdminPageFrameworkRepeatableFields();
        ...
        // For nested fields,
        $( nodeNewField ).find( '.yucca-shop-field' ).each( function( iIterationIndex ) {    
    ...
    $.fn.removeyucca_shopAdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.yucca-shop-field' );
        var nodeFieldsContainer = $( this ).closest( '.yucca-shop-fields' );
        var sFieldsContainerID  = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.yucca-shop-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.yucca-shop-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.ayucca_shopAdminPageFrameworkRepeatableFieldsOptions[ sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
