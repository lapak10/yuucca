...
class yucca_shopAdminPageFramework_Form_View___Script_RepeatableSection extends yucca_shopAdminPageFramework_Form_View___Script_Base
{
...
( function( $ ) {
     ...
     */
    $.fn.updateyucca_shopAdminPageFrameworkRepeatableSections = function( aSettings ) {
// @todo Change the selector name 'repeatable-section-add-button' to something else to avoid apf version conflict.
        var _oThis                = this; 
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.yucca-shop-sections' ).attr( 'id' );

        // Store the sections specific options in an array.
        if ( ! $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions ) $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions = [];
        if ( ! $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {     
            $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).click( function() {
            $( this ).addyucca_shopAdminPageFrameworkRepeatableSection();
        ...
        $( _oThis ).find( '.repeatable-section-remove-button' ).click( function() {
            $( this ).removeyucca_shopAdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });     
        
        // If the number of sections is less than the set minimum value, add sections. 
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.yucca-shop-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.yucca-shop-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {     
                $( '#' + _sSectionID ).addyucca_shopAdminPageFrameworkRepeatableSection( _sSectionID );  
            }
        }
     ...
     */    
    $.fn.addyucca_shopAdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        
        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.yucca-shop-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.yucca-shop-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.yucca-shop-section-tabs-contents' )
            .children( '.yucca-shop-section-tabs' )
            .first();
            
        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );
        
        var _iFadein                = $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        
        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.yucca-shop-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.yucca-shop-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.yucca-shop-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.yucca-shop-section-title' ).not( '.yucca-shop-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enableyucca_shopAdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.yucca-shop-collapsible-sections-title, .yucca-shop-collapsible-section-title' ).enableyucca_shopAdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.yucca-shop-field' ).each( function( iFieldIndex ) {    
            ...
            $( this ).updateyucca_shopAdminPageFrameworkRepeatableFields();
        ...
        nodeNewSection.updateyucca_shopAdminPageFrameworkRepeatableSections();    
        
        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.yucca-shop-fields.sortable' ).each( function() {
            $( this ).enableyucca_shopAdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.yucca-shop-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.yucca-shop-fieldrow, .yucca-shop-fieldset, .yucca-shop-fields, .yucca-shop-field, table.form-table, input,textarea,select' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.yucca-shop-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.yucca-shop-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.yucca-shop-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removeyucca_shopAdminPageFrameworkRepeatableSection = function() {
        
        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.yucca-shop-section' );
        var sSectionConteinrID      = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.yucca-shop-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.yucca-shop-section-tabs-contents' )
            .children( '.yucca-shop-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.yucca-shop-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.ayucca_shopAdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.yucca-shop-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.yucca-shop-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.yucca-shop-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.yucca-shop-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.yucca-shop-field' ).each( function( iFieldIndex ) {    
...
JAVASCRIPTS;
    ...
    private static $_aSetContainerIDsForRepeatableSections = [];
    ...
    public static function getEnabler($sContainerTagID, $iSectionCount, $aSettings, $oMsg)
    {
        ...
        new self($oMsg);
        $aSettings = self::getAsArray($aSettings) + ['min' => 0, 'max' => 0];
...
jQuery( document ).ready( function() {
    ...
    // Update the fields     
    jQuery( '#{$sContainerTagID}' ).updateyucca_shopAdminPageFrameworkRepeatableSections( $_aJSArray ); 
});            
JAVASCRIPTS;

        return "<script type='text/javascript' class='yucca-shop-section-repeatable-script'>".'/* <![CDATA[ */'.$_sScript.'/* ]]> */'.'</script>';
    }
}
