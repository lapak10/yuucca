...
class yucca_shopAdminPageFramework_Form_Model___FormatFieldsets extends yucca_shopAdminPageFramework_Form_Base
{
    public $aSectionsets = [];
    public $aFieldsets = [];
    public $sStructureType = '';
    public $sCapability = '';
    public $aCallbacks = ['fieldset_before_output' => null];
    public $aSavedData = [];
    public $oCallerForm;

    public function __construct()
    {
        $_aParameters = func_get_args() + [$this->aFieldsets, $this->aSectionsets, $this->sStructureType, $this->aSavedData, $this->sCapability, $this->aCallbacks, $this->oCallerForm];
    ...
    {
        $_oDynamicElements = new yucca_shopAdminPageFramework_Form_Model___FormatDynamicElements($this->aSectionsets, $this->aFieldsets, $this->aSavedData);
    ...
    private function _getFieldsetsFormatted(array $aFieldsets, array $aSectionsets, $sCapability)
    {
        ...
        foreach ($aFieldsets as $_sSectionPath => $_aItems) {
            ...
            $_aNewFieldsets[$_sSectionPath] = $this->_getItemsFormatteed($_sSectionPath, $_aItems, $this->getElement($aSectionsets, [$_sSectionPath, 'capability'], $sCapability), $aSectionsets);
        }
        $this->_sortFieldsBySectionsOrder($_aNewFieldsets, $aSectionsets);

        return $this->callBack($this->aCallbacks['fieldsets_after_formatting'], [$_aNewFieldsets, $aSectionsets]);
    }

    private function _getItemsFormatteed($sSectionPath, $aItems, $sCapability, $aSectionsets)
    {
        $_abSectionRepeatable = $this->getElement($aSectionsets, [$sSectionPath, 'repeatable'], false);
    ...
    private function _getNormalFieldsetsFormatted($aItems, $sCapability, $aSectionsets, $_abSectionRepeatable)
    {
        ...
        uasort($_aNewItems, [$this, 'sortArrayByKey']);
    ...
    private function _getSubSectionsFormatted($aItems, $sCapability, $aSectionsets, $_abSectionRepeatable)
    {
        ...
        foreach ($this->numerizeElements($aItems) as $_iSubSectionIndex => $_aFieldsets) {
            ...
            uasort($_aNewFieldset[$_iSubSectionIndex], [$this, 'sortArrayByKey']);
        }
    ...
    private function _getFieldsetFormatted($aFieldset, $aSectionsets, $sCapability, $iCountOfElements, $iSubSectionIndex, $bIsSectionRepeatable, $oCallerObject)
    {
        ...
        $_oFieldsetFormatter = new yucca_shopAdminPageFramework_Form_Model___Format_Fieldset($aFieldset, $this->sStructureType, $sCapability, $iCountOfElements, $iSubSectionIndex, $bIsSectionRepeatable, $oCallerObject);
        $_aFieldset = $this->callBack($this->aCallbacks['fieldset_before_output'], [$_oFieldsetFormatter->get(), $aSectionsets]);
        $_aFieldset = $this->callBack($this->aCallbacks['fieldset_after_formatting'], [$_aFieldset, $aSectionsets]);
