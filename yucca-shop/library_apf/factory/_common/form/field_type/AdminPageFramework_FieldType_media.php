...
class yucca_shopAdminPageFramework_FieldType_image extends yucca_shopAdminPageFramework_FieldType
{
    public $aFieldTypeSlugs = ['image'];
    protected $aDefaultKeys = ['attributes_to_store' => [], 'show_preview' => true, 'allow_external_source' => true, 'attributes' => ['input' => ['size' => 40, 'maxlength' => 400], 'button' => [], 'remove_button' => [], 'preview' => []]];
...
jQuery( document ).ready( function(){
    ...
    jQuery().registeryucca_shopAdminPageFrameworkCallbacks( {   
        ...
        added_repeatable_field: function( oCloned, sFieldType, sFieldTagID, iCallType, iSectionIndex, iFieldIndex ) {
            ...
            // Bind the event.
            var _oFieldContainer = oCloned.closest( '.yucca-shop-field' );
            ...
            setyucca_shopAdminPageFrameworkImageUploader( 
 ...
 */
setyucca_shopAdminPageFrameworkImageUploader = function( sInputID, fMultiple, fExternalSource ) {
    ...
    jQuery( '#select_image_' + sInputID ).click( function() {
        ...
        window.send_to_editor           = hfyucca_shopAdminPageFrameworkSendToEditorImage;
...
var hfyucca_shopAdminPageFrameworkSendToEditorImage = function( sRawHTML ) {
...
setyucca_shopAdminPageFrameworkImageUploader = function( sInputID, fMultiple, fExternalSource ) {
    ...
    jQuery( '#select_image_' + sInputID ).click( function( e ) {
        ...
        oyucca_shopAdminPageFrameworkOriginalImageUploaderSelectObject = wp.media.view.MediaFrame.Select;
        
        // Assign a custom select object
        wp.media.view.MediaFrame.Select = fExternalSource ? getyucca_shopAdminPageFrameworkCustomMediaUploaderSelectObject() : oyucca_shopAdminPageFrameworkOriginalImageUploaderSelectObject;
        ...
        _oCustomImageUploader.on( 'close', function() {
            ...
            if ( typeof( _oImage ) !== 'undefined'  ) {
                ...
                _oCustomImageUploader.state().get( 'selection' ).each( function( oAttachment, iIndex ) {
                    ...
                    var _oFieldContainer    = 'undefined' === typeof _oNewField 
                        ? jQuery( '#' + sInputID ).closest( '.yucca-shop-field' ) 
                        : _oNewField;
                    _oNewField              = jQuery( this ).addyucca_shopAdminPageFrameworkRepeatableField( _oFieldContainer.attr( 'id' ) );
            ...
            wp.media.view.MediaFrame.Select = oyucca_shopAdminPageFrameworkOriginalImageUploaderSelectObject;
...
removeInputValuesForImage = function( oElem ) {
    ...
    var _oImageInput = jQuery( oElem ).closest( '.yucca-shop-field' ).find( '.image-field input' );                  
    if ( _oImageInput.length <= 0 )  {
        return;
    }
...
JAVASCRIPTS;
    ...
    {
        return '.yucca-shop-field .image_preview {border: none; clear: both; margin-top: 0.4em;margin-bottom: 0.8em;display: block; max-width: 100%;height: auto; width: inherit;} .yucca-shop-field .image_preview img { display: block;height: auto; max-width: 100%;}.widget .yucca-shop-field .image_preview {max-width: 100%;}@media only screen and ( max-width: 1200px ) {.yucca-shop-field .image_preview {max-width: 600px;} } @media only screen and ( max-width: 900px ) {.yucca-shop-field .image_preview {max-width: 440px;}}@media only screen and ( max-width: 600px ) {.yucca-shop-field .image_preview {max-width: 300px;}} @media only screen and ( max-width: 480px ) {.yucca-shop-field .image_preview {max-width: 240px;}}@media only screen and ( min-width: 1200px ) {.yucca-shop-field .image_preview {max-width: 600px;}}.yucca-shop-field-image input {margin-right: 0.5em;vertical-align: middle;}.select_image.button.button-small,.remove_image.button.button-small{ vertical-align: middle;}.remove_image.button.button-small {margin-left: 0.2em;}@media screen and (max-width: 782px) {.yucca-shop-field-image input {margin: 0.5em 0.5em 0.5em 0;}} ';
    }
    ...
    protected function getField($aField)
    {
        ...
        return $aField['before_label']."<div class='yucca-shop-input-label-container yucca-shop-input-container {$aField['type']}-field'>"."<label for='{$aField['input_id']}'>".$aField['before_input'].$this->getAOrB($aField['label'] && !$aField['repeatable'], "<span class='yucca-shop-input-label-string' style='min-width:".$this->sanitizeLength($aField['label_min_width']).";'>".$aField['label'].'</span>', '').'<input '.$this->getAttributes($this->_getImageInputAttributes($aField, $_iCountAttributes, $_sImageURL, $_aBaseAttributes)).' />'.$aField['after_input']."<div class='repeatable-field-buttons'></div>".$this->getExtraInputFields($aField).'</label>'.'</div>'.$aField['after_label'].$this->_getPreviewContainer($aField, $_sImageURL, $this->getElementAsArray($aField, ['attributes', 'preview']) + $_aBaseAttributes).$this->_getRemoveButtonScript($aField['input_id'], $this->getElementAsArray($aField, ['attributes', 'remove_button']) + $_aBaseAttributes, $aField['type']).$this->_getUploaderButtonScript($aField['input_id'], $aField['repeatable'], $aField['allow_external_source'], $this->getElementAsArray($aField, ['attributes', 'button']) + $_aBaseAttributes);
    }

    private function _getBaseAttributes(array $aField)
    {
        $_aBaseAttributes = $aField['attributes'] + ['class' => null];
    ...
    private function _getTheSetImageURL(array $aField, $iCountAttributes)
    {
        ...
        return $_sCaptureAttribute ? $this->getElement($aField, ['attributes', 'value', $_sCaptureAttribute], '') : $aField['attributes']['value'];
    }

    private function _getImageInputAttributes(array $aField, $iCountAttributes, $sImageURL, array $aBaseAttributes)
    {
        return ['name' => $aField['attributes']['name'].$this->getAOrB($iCountAttributes, '[url]', ''), 'value' => $sImageURL, 'type' => 'text', 'data-show_preview' => $aField['show_preview']] + $aField['attributes']['input'] + $aBaseAttributes;
    }
    ...
    protected function getExtraInputFields(array $aField)
    {
        ...
        foreach ($this->getElementAsArray($aField, 'attributes_to_store') as $sAttribute) {
            $_aOutputs[] = '<input '.$this->getAttributes(['id' => "{$aField['input_id']}_{$sAttribute}", 'type' => 'hidden', 'name' => "{$aField['_input_name']}[{$sAttribute}]", 'disabled' => $this->getAOrB(isset($aField['attributes']['disabled']) && $aField['attributes']['disabled'], 'disabled', null), 'value' => $this->getElement($aField, ['attributes', 'value', $sAttribute], '')]).'/>';
        }
    ...
    protected function _getPreviewContainer($aField, $sImageURL, $aPreviewAtrributes)
    {
        ...
        return '<div '.$this->getAttributes(['id' => "image_preview_container_{$aField['input_id']}", 'class' => 'image_preview '.$this->getElement($aPreviewAtrributes, 'class', ''), 'style' => $this->getAOrB($sImageURL, '', 'display: none; ').$this->getElement($aPreviewAtrributes, 'style', '')] + $aPreviewAtrributes).'>'."<img src='{$sImageURL}' "."id='image_preview_{$aField['input_id']}' ".'/>'.'</div>';
    }
...
jQuery( document ).ready( function(){     
    setyucca_shopAdminPageFrameworkImageUploader( '{$sInputID}', '{$bRpeatable}', '{$bExternalSource}' );
});
JAVASCRIPTS;

        return "<script type='text/javascript' class='yucca-shop-image-uploader-button'>".'/* <![CDATA[ */'.$_sScript.'/* ]]> */'.'</script>'.PHP_EOL;
    }
    ...
    {
        $_aAttributes = ['id' => "select_image_{$sInputID}", 'href' => '#', 'data-uploader_type' => (string) function_exists('wp_enqueue_media'), 'data-enable_external_source' => (string) (bool) $bExternalSource] + $aButtonAttributes + ['title' => $_bIsLabelSet ? $aButtonAttributes['data-label'] : $this->oMsg->get('select_image'), 'data-label' => null];
...
JAVASCRIPTS;
        ...
        return "<script type='text/javascript' class='yucca-shop-{$sType}-remove-button'>".'/* <![CDATA[ */'.$_sScript.'/* ]]> */'.'</script>'.PHP_EOL;
    }
    ...
    protected function _getFormattedRemoveButtonAttributesByType($sInputID, array $aButtonAttributes, $_bIsLabelSet, $sType = 'image')
    {
        ...
        $_aAttributes = ['id' => "remove_{$sType}_{$sInputID}", 'href' => '#', 'onclick' => esc_js("{$_sOnClickFunctionName}( this ); return false;")] + $aButtonAttributes + ['title' => $_bIsLabelSet ? $aButtonAttributes['data-label'] : $this->oMsg->get('remove_value')];
...
class yucca_shopAdminPageFramework_FieldType_media extends yucca_shopAdminPageFramework_FieldType_image
{
    public $aFieldTypeSlugs = ['media'];
    protected $aDefaultKeys = ['attributes_to_store' => [], 'show_preview' => true, 'allow_external_source' => true, 'attributes' => ['input' => ['size' => 40, 'maxlength' => 400], 'button' => [], 'remove_button' => [], 'preview' => []]];
...
jQuery( document ).ready( function(){
    ...
    jQuery().registeryucca_shopAdminPageFrameworkCallbacks( {    
        ...
        added_repeatable_field: function( oCloned, sFieldType, sFieldTagID, iCallType ) {
            ...
            setyucca_shopAdminPageFrameworkMediaUploader( 
    ...
    private function _getScript_MediaUploader($sReferrer)
    {
        ...
        if (!function_exists('wp_enqueue_media')) {
            return <<<JAVASCRIPTS
                     ...
                     * The fMultiple parameter does not do anything. It is there to be consistent with the one for the WordPress version 3.5 or above.
                     */
                    setyucca_shopAdminPageFrameworkMediaUploader = function( sInputID, fMultiple, fExternalSource ) {
                        ...
                        jQuery( '#select_media_' + sInputID ).click( function() {
                            ...
                            window.original_send_to_editor = window.send_to_editor;
                            window.send_to_editor = hfyucca_shopAdminPageFrameworkSendToEditorMedia;
                            var fExternalSource = jQuery( this ).attr( 'data-enable_external_source' );
                            tb_show( '{$_sThickBoxTitle}', 'media-upload.php?post_id=1&amp;enable_external_source=' + fExternalSource + '&amp;referrer={$sReferrer}&amp;button_label={$_sThickBoxButtonUseThis}&amp;type=image&amp;TB_iframe=true', false );
                            return false; // do not click the button after the script by returning false.     
                        });    
                    ...
                    var hfyucca_shopAdminPageFrameworkSendToEditorMedia = function( sRawHTML, param ) {
        ...
        return <<<JAVASCRIPTS
                ...
                setyucca_shopAdminPageFrameworkMediaUploader = function( sInputID, fMultiple, fExternalSource ) {
                    ...
                    jQuery( '#select_media_' + sInputID ).click( function( e ) {
                        ...
                        // Store the original select object in a global variable
                        oyucca_shopAdminPageFrameworkOriginalMediaUploaderSelectObject = wp.media.view.MediaFrame.Select;
                        
                        // Assign a custom select object.
                        wp.media.view.MediaFrame.Select = fExternalSource ? getyucca_shopAdminPageFrameworkCustomMediaUploaderSelectObject() : oyucca_shopAdminPageFrameworkOriginalMediaUploaderSelectObject;
                        ...
                        _oMediaUploader.on( 'close', function() {
                            ...
                            // Restore the original select object.
                            wp.media.view.MediaFrame.Select = oyucca_shopAdminPageFrameworkOriginalMediaUploaderSelectObject;    
                ...
                removeInputValuesForMedia = function( oElem ) {
                    ...
                    var _oImageInput = jQuery( oElem ).closest( '.yucca-shop-field' ).find( '.media-field input' );                  
                    if ( _oImageInput.length <= 0 )  {
                        return;
                    }
...
jQuery( document ).ready( function(){     
    setyucca_shopAdminPageFrameworkMediaUploader( '{$sInputID}', '{$bRpeatable}', '{$bExternalSource}' );
});
JAVASCRIPTS;

        return "<script type='text/javascript' class='yucca-shop-media-uploader-button'>".'/* <![CDATA[ */'.$_sScript.'/* ]]> */'.'</script>'.PHP_EOL;
    }
    ...
    {
        $_aAttributes = ['id' => "select_media_{$sInputID}", 'href' => '#', 'data-uploader_type' => (string) function_exists('wp_enqueue_media'), 'data-enable_external_source' => (string) (bool) $bExternalSource] + $aButtonAttributes + ['title' => $_bIsLabelSet ? $aButtonAttributes['data-label'] : $this->oMsg->get('select_file'), 'data-label' => null];
