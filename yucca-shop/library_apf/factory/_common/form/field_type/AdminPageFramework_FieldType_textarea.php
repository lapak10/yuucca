...
class yucca_shopAdminPageFramework_FieldType_textarea extends yucca_shopAdminPageFramework_FieldType
{
    public $aFieldTypeSlugs = ['textarea'];
    protected $aDefaultKeys = ['rich' => false, 'attributes' => ['autofocus' => null, 'cols' => 60, 'disabled' => null, 'formNew' => null, 'maxlength' => null, 'placeholder' => null, 'readonly' => null, 'required' => null, 'rows' => 4, 'wrap' => null]];
...
jQuery( document ).ready( function(){
    ...
    jQuery().registeryucca_shopAdminPageFrameworkCallbacks( {				
        ...
        added_repeatable_field: function( oCloned, sFieldType, sFieldTagID, iCallType, iSectionIndex, iFieldIndex ) {
            ...
            var _oSettings = jQuery().getyucca_shopAdminPageFrameworkInputOptions( _oWrap.attr( 'data-id' ) );  

            // Elements
            var _oField              = oCloned.closest( '.yucca-shop-field' );
        ...
        stopped_sorting_fields : function( oSortedFields, sFieldType, sFieldsTagID, iCallType ) { 
            ...
            // Update the editor.
            oSortedFields.children( '.yucca-shop-field' ).each( function( iIndex ) {
                ...
                var oSettings = jQuery().getyucca_shopAdminPageFrameworkInputOptions( oWrap.attr( 'data-id' ) );   
        ...
        saved_widget : function( oWidget ) { 
            ...
            jQuery( oWidget ).find( '.yucca-shop-field' ).each( function( iIndex ) {
                ...
                // Store the settings.
                jQuery().storeyucca_shopAdminPageFrameworkInputOptions( 
                    oWrap.attr( 'data-id' ), 
                    { 
...
JAVASCRIPTS;
    ...
    {
        return '.yucca-shop-field-textarea .yucca-shop-input-label-string {vertical-align: top;margin-top: 2px;} .yucca-shop-field-textarea .wp-core-ui.wp-editor-wrap {margin-bottom: 0.5em;}.yucca-shop-field-textarea.yucca-shop-field .yucca-shop-input-label-container {vertical-align: top; float: left;clear: both;} .postbox .yucca-shop-field-textarea .yucca-shop-input-label-container {width: 100%;}.yucca-shop-field-textarea textarea {max-width: 100%;}';
    }
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField)
    {
        ...
        $_sClassSelector = $_bIsArray ? 'yucca-shop-field-textarea-multiple-labels' : '';
        ...
        $aField['attributes'] = $_bIsArray ? ['name' => $aField['attributes']['name']."[{$sKey}]", 'id' => $aField['attributes']['id']."_{$sKey}", 'value' => $aField['value']] + $aField['attributes'] : $aField['attributes'];
        $_aOutput = [$this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='yucca-shop-input-label-container {$_sClassSelector}'>", "<label for='".$aField['attributes']['id']."'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span class='yucca-shop-input-label-string' style='min-width:".$this->sanitizeLength($aField['label_min_width']).";'>".$_sLabel.'</span>' : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), '</label>', '</div>', $this->getElementByLabel($aField['after_label'], $sKey, $aField['label'])];
    ...
    private function _getEditor($aField)
    {
        ...
        ob_start();
        wp_editor($aField['value'], $aField['attributes']['id'], $this->uniteArrays((array) $aField['rich'], ['wpautop' => true, 'media_buttons' => true, 'textarea_name' => $aField['attributes']['name'], 'textarea_rows' => $aField['attributes']['rows'], 'tabindex' => '', 'tabfocus_elements' => ':prev,:next', 'editor_css' => '', 'editor_class' => $aField['attributes']['class'], 'teeny' => false, 'dfw' => false, 'tinymce' => true, 'quicktags' => true]));
...
JAVASCRIPTS;
        ...
        return "<script type='text/javascript' class='yucca-shop-textarea-enabler'>".'/* <![CDATA[ */'.$_sScript.'/* ]]> */'.'</script>';
    }
}
