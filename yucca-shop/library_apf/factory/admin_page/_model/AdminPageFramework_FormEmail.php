...
class yucca_shopAdminPageFramework_FormEmail extends yucca_shopAdminPageFramework_FrameworkUtility
{
    public $aEmailOptions = [];
    public $aInput = [];
    public $sSubmitSectionID;
    private $_aPathsToDelete = [];
    ...
    public function send()
    {
        ...
        if ($_bIsHTML = $this->_getEmailArgument($aInput, $aEmailOptions, 'is_html', $sSubmitSectionID)) {
            add_filter('wp_mail_content_type', [$this, '_replyToSetMailContentTypeToHTML']);
        }
        if ($this->_sEmailSenderAddress = $this->_getEmailArgument($aInput, $aEmailOptions, 'from', $sSubmitSectionID)) {
            add_filter('wp_mail_from', [$this, '_replyToSetEmailSenderAddress']);
        }
        if ($this->_sEmailSenderName = $this->_getEmailArgument($aInput, $aEmailOptions, 'name', $sSubmitSectionID)) {
            add_filter('wp_mail_from_name', [$this, '_replyToSetEmailSenderAddress']);
        }
        ...
        remove_filter('wp_mail_content_type', [$this, '_replyToSetMailContentTypeToHTML']);
        remove_filter('wp_mail_from', [$this, '_replyToSetEmailSenderAddress']);
        remove_filter('wp_mail_from_name', [$this, '_replyToSetEmailSenderAddress']);
    ...
    private function _getEmailArgument($aInput, array $aEmailOptions, $sKey, $sSectionID)
    {
        ...
        if (!$aEmailOptions[$sKey]) {
            return $this->getArrayValueByArrayKeys($aInput, [$sSectionID, $sKey]);
        }
