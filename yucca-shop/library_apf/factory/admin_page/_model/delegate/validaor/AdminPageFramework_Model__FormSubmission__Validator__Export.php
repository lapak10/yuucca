...
class yucca_shopAdminPageFramework_Model__FormSubmission__Validator__Import extends yucca_shopAdminPageFramework_Model__FormSubmission__Validator_Base
{
    ...
    {
        $_oImport = new yucca_shopAdminPageFramework_ImportOptions($_FILES['__import'], $_POST['__import']);
        $_aArguments = ['class_name' => $this->oFactory->oProp->sClassName, 'page_slug' => $sPageSlug, 'tab_slug' => $sTabSlug, 'section_id' => $_oImport->getSiblingValue('section_id'), 'pressed_field_id' => $_oImport->getSiblingValue('field_id'), 'pressed_input_id' => $_oImport->getSiblingValue('input_id'), 'should_merge' => $_oImport->getSiblingValue('is_merge')];
    ...
    {
        return $this->_getFilteredItemForPortByPrefix('import_mime_types_', ['text/plain', 'application/octet-stream', 'application/json', 'text/html', 'application/txt'], $aArguments);
    }
    ...
    {
        return [$sPrefix.$aArguments['class_name'].'_'.$aArguments['pressed_input_id'], $aArguments['section_id'] ? $sPrefix.$aArguments['class_name'].'_'.$aArguments['section_id'].'_'.$aArguments['pressed_field_id'] : $sPrefix.$aArguments['class_name'].'_'.$aArguments['pressed_field_id'], $aArguments['section_id'] ? $sPrefix.$aArguments['class_name'].'_'.$aArguments['section_id'] : null, $aArguments['tab_slug'] ? $sPrefix.$aArguments['page_slug'].'_'.$aArguments['tab_slug'] : null, $sPrefix.$aArguments['page_slug'], $sPrefix.$aArguments['class_name']];
    }
}
class yucca_shopAdminPageFramework_Model__FormSubmission__Validator__Export extends yucca_shopAdminPageFramework_Model__FormSubmission__Validator__Import
{
    public $sActionHookPrefix = 'try_validation_after_';
    ...
    {
        $_oExport = new yucca_shopAdminPageFramework_ExportOptions($_POST['__export'], $this->oFactory->oProp->sClassName);
        $_aArguments = ['class_name' => $this->oFactory->oProp->sClassName, 'page_slug' => $sPageSlug, 'tab_slug' => $sTabSlug, 'section_id' => $_oExport->getSiblingValue('section_id'), 'pressed_field_id' => $_oExport->getSiblingValue('field_id'), 'pressed_input_id' => $_oExport->getSiblingValue('input_id')];
    ...
    {
        $_aHeader = ['Content-Description' => 'File Transfer', 'Content-Disposition' => "attachment; filename=\"{$sFileName}\";"];
