...
class yucca_shopAdminPageFramework_Model__FormSubmission__Validator__ContactForm extends yucca_shopAdminPageFramework_Model__FormSubmission__Validator_Base
{
    ...
    public function _replyToCallback($aInputs, $aRawInputs, array $aSubmits, $aSubmitInformation, $oFactory)
    {
        ...
        add_action("setting_update_url_{$this->oFactory->oProp->sClassName}", [$this, '_replyToRemoveConfirmationQueryKey']);
    ...
    private function _sendEmailInBackground($aInputs, $sPressedInputNameFlat, $sSubmitSectionID)
    {
        ...
        $_aEmailOptions = $this->getAsArray($_aEmailOptions) + ['to' => '', 'subject' => '', 'message' => '', 'headers' => '', 'attachments' => '', 'is_html' => false, 'from' => '', 'name' => ''];
        $_sTransientKey = 'apf_emd_'.md5($sPressedInputNameFlat.get_current_user_id());
        $_aFormEmailData = ['email_options' => $_aEmailOptions, 'input' => $aInputs, 'section_id' => $sSubmitSectionID];
        $_bIsSet = $this->setTransient($_sTransientKey, $_aFormEmailData, 100);
        wp_remote_get(add_query_arg(['apf_action' => 'email', 'transient' => $_sTransientKey], admin_url($GLOBALS['pagenow'])), ['timeout' => 0.01, 'sslverify' => false]);
        $_bSent = $_bIsSet;
    ...
    {
        return remove_query_arg(['confirmation'], $sSettingUpdateURL);
    }
}
class yucca_shopAdminPageFramework_Model__FormSubmission__Validator__ContactFormConfirm extends yucca_shopAdminPageFramework_Model__FormSubmission__Validator__ContactForm
{
    public $sActionHookPrefix = 'try_validation_after_';
    ...
    {
        return ['confirmation' => 'email'] + $aStatus;
    }
}
