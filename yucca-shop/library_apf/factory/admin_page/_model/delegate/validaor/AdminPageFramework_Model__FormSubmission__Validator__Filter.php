...
class yucca_shopAdminPageFramework_Model__FormSubmission__Validator__Filter extends yucca_shopAdminPageFramework_Model__FormSubmission_Base
{
    public $oFactory;
    public $aInputs = [];
    public $aRawInputs = [];
    public $aOptions = [];
    public $aSubmitInformation = [];
    private $_bHasFieldErrors = false;

    public function __construct()
    {
        $_aParameters = func_get_args() + [$this->oFactory, $this->aInputs, $this->aRawInputs, $this->aOptions, $this->aSubmitInformation];
    ...
    {
        $_aData = ['sPageSlug' => $aSubmitInformation['page_slug'], 'sTabSlug' => $aSubmitInformation['tab_slug'], 'aInput' => $this->getAsArray($aInputs), 'aStoredData' => $aStoredData, 'aStoredTabData' => [], 'aStoredDataWODynamicElements' => $this->addAndApplyFilter($this->oFactory, "validation_saved_options_without_dynamic_elements_{$this->oFactory->oProp->sClassName}", $this->oFactory->oForm->dropRepeatableElements($aStoredData), $this->oFactory), 'aStoredTabDataWODynamicElements' => [], 'aEmbeddedDataWODynamicElements' => [], 'aSubmitInformation' => $aSubmitInformation];
        ...
        $_aInput = $this->_getValidatedData("validation_{$this->oFactory->oProp->sClassName}", call_user_func_array([$this->oFactory, 'validate'], [$_aData['aInput'], $_aData['aStoredData'], $this->oFactory, $_aData['aSubmitInformation']]), $_aData['aStoredData'], $_aData['aSubmitInformation']);
    ...
    {
        return ['field_errors' => $this->_bHasFieldErrors] + $aStatus;
    }
        ...
        foreach ($aInputsToParse as $_sID => $_aSectionOrFields) {
            if ($this->oFactory->oForm->isSection($_sID)) {
                ...
                foreach ($_aSectionOrFields as $_sFieldID => $_aFields) {
                    $aData['aInput'][$_sID][$_sFieldID] = $this->_getValidatedData("validation_{$this->oFactory->oProp->sClassName}_{$_sID}_{$_sFieldID}", $aData['aInput'][$_sID][$_sFieldID], $this->getElement($aData, ['aStoredData', $_sID, $_sFieldID], null), $aData['aSubmitInformation']);
                }
                $_aSectionInput = is_array($aData['aInput'][$_sID]) ? $aData['aInput'][$_sID] : [];
                $_aSectionInput = $_aSectionInput + (isset($aData['aStoredDataWODynamicElements'][$_sID]) && is_array($aData['aStoredDataWODynamicElements'][$_sID]) ? $aData['aStoredDataWODynamicElements'][$_sID] : []);
                $aData['aInput'][$_sID] = $this->_getValidatedData("validation_{$this->oFactory->oProp->sClassName}_{$_sID}", $_aSectionInput, $this->getElement($aData, ['aStoredData', $_sID], null), $aData['aSubmitInformation']);
                continue;
            }
            if (!$this->_isValidSection('_default', $aData['sPageSlug'], $aData['sTabSlug'])) {
                continue;
            }
            $aData['aInput'][$_sID] = $this->_getValidatedData("validation_{$this->oFactory->oProp->sClassName}_{$_sID}", $aData['aInput'][$_sID], $this->getElement($aData, ['aStoredData', $_sID], null), $aData['aSubmitInformation']);
        }
    ...
    private function _validatePageFields(array $aData)
    {
        ...
        $_aPageOptions = $aData['sTabSlug'] && !empty($aData['aStoredTabData']) ? $this->invertCastArrayContents($_aPageOptions, $aData['aStoredTabData']) : (!$aData['sTabSlug'] ? [] : $_aPageOptions);
    ...
    private function _getValidatedData($sFilterName, $aInputs, $aStoredData, $aSubmitInfo = [])
    {
