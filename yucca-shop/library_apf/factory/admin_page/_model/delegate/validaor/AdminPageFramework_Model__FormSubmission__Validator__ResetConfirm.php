...
class yucca_shopAdminPageFramework_Model__FormSubmission__Validator__Reset extends yucca_shopAdminPageFramework_Model__FormSubmission__Validator_Base
{
    ...
    private function _resetOptions($sKeyToReset, array $aInputs, array $aSubmitInformation)
    {
        ...
        if (in_array($sKeyToReset, ['1'], true)) {
    ...
    private function _doResetActions($sKeyToReset, $aInputs, $aSubmitInformation)
    {
        ...
        $this->addAndDoActions($this->oFactory, [$_sSectionID ? "reset_{$this->oFactory->oProp->sClassName}_{$_sSectionID}_{$_sFieldID}" : "reset_{$this->oFactory->oProp->sClassName}_{$_sFieldID}", $_sSectionID ? "reset_{$this->oFactory->oProp->sClassName}_{$_sSectionID}" : null, $_sTabSlug ? "reset_{$this->oFactory->oProp->sClassName}_{$_sPageSlug}_{$_sTabSlug}" : null, "reset_{$this->oFactory->oProp->sClassName}_{$_sPageSlug}", "reset_{$this->oFactory->oProp->sClassName}"], $sKeyToReset, $aInputs, $this->oFactory, $aSubmitInformation);
    }
}
class yucca_shopAdminPageFramework_Model__FormSubmission__Validator__ResetConfirm extends yucca_shopAdminPageFramework_Model__FormSubmission__Validator__Reset
{
    public $sActionHookPrefix = 'try_validation_after_';
    ...
    {
        return ['confirmation' => 'reset'] + $aStatus;
    }
}
