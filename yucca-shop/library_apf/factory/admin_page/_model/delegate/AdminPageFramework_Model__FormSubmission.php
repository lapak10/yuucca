...
abstract class yucca_shopAdminPageFramework_Model__FormSubmission_Base extends yucca_shopAdminPageFramework_FrameworkUtility
{
...
class yucca_shopAdminPageFramework_Model__FormSubmission extends yucca_shopAdminPageFramework_Model__FormSubmission_Base
{
    public $oFactory;
    ...
    public function __construct($oFactory, $aSavedData, $aArguments, $aSectionsets, $aFieldsets)
    {
        ...
        new yucca_shopAdminPageFramework_Model__FormRedirectHandler($oFactory);
    }
    ...
    public function _handleFormData()
    {
        ...
        $_aSubmits = $this->getElementAsArray($_POST, '__submit', []);
        ...
        new yucca_shopAdminPageFramework_Model__FormSubmission__Validator($this->oFactory);
        ...
        $this->goToLocalURL($this->_getSettingUpdateURL(['settings-updated' => true], $_sPageSlug, $_sTabSlug));
    }
    ...
    private function _shouldProceed()
    {
        ...
        $_sRequestURI = remove_query_arg(['settings-updated', 'confirmation', 'field_errors'], wp_unslash($_SERVER['REQUEST_URI']));
        $_sReffererURI = remove_query_arg(['settings-updated', 'confirmation', 'field_errors'], $_POST['_wp_http_referer']);
    ...
    {
        $_aInputs = $this->getElementAsArray($_POST, $this->oFactory->oProp->sOptionKey, []);
    ...
    {
        $this->addAndDoActions($this->oFactory, [$this->getAOrB($_sSubmitSectionID, "submit_after_{$this->oFactory->oProp->sClassName}_{$_sSubmitSectionID}_{$_sPressedFieldID}", "submit_after_{$this->oFactory->oProp->sClassName}_{$_sPressedFieldID}"), $this->getAOrB($_sSubmitSectionID, "submit_after_{$this->oFactory->oProp->sClassName}_{$_sSubmitSectionID}", null), $this->getAOrB(isset($_POST['tab_slug']), "submit_after_{$this->oFactory->oProp->sClassName}_{$_sPageSlug}_{$_sTabSlug}", null), "submit_after_{$this->oFactory->oProp->sClassName}_{$_sPageSlug}", "submit_after_{$this->oFactory->oProp->sClassName}"], $_bUpdated ? $_aInputs : [], $_aOptions, $this->oFactory);
    }

    private function _getSettingUpdateURL(array $aStatus, $sPageSlug, $sTabSlug)
    {
        $aStatus = $this->addAndApplyFilters($this->oFactory, ["options_update_status_{$sPageSlug}_{$sTabSlug}", "options_update_status_{$sPageSlug}", "options_update_status_{$this->oFactory->oProp->sClassName}"], $aStatus);
        $_aRemoveQueries = [];
        ...
        return $this->addAndApplyFilters($this->oFactory, ["setting_update_url_{$this->oFactory->oProp->sClassName}"], $this->getQueryURL($aStatus, $_aRemoveQueries, $_SERVER['REQUEST_URI']));
    }
