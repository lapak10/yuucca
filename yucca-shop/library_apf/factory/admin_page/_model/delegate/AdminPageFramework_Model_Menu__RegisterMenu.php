...
class yucca_shopAdminPageFramework_Model_Menu__RegisterMenu extends yucca_shopAdminPageFramework_FrameworkUtility
{
    public $oFactory;
    ...
    public function __construct($oFactory, $sActionHook = 'admin_menu')
    {
        ...
        add_action($sActionHook, [$this, '_replyToRegisterMenu'], 98);
        add_action($sActionHook, [$this, 'sortAdminSubMenu'], 9999);
        $GLOBALS['_apf_sub_menus_to_sort'] = isset($GLOBALS['_apf_sub_menus_to_sort']) ? $GLOBALS['_apf_sub_menus_to_sort'] : [];
    }
    ...
    public function _replyToRegisterMenu()
    {
        ...
        foreach ($this->oFactory->oProp->aPages as &$aSubMenuItem) {
            $_oFormatter = new yucca_shopAdminPageFramework_Format_SubMenuItem($aSubMenuItem, $this->oFactory, $_iParsedIndex);
    ...
    private static $_aRootMenuSlugCache = [];
    ...
    private function _addPageSubmenuItem($sRootPageSlug, $sMenuSlug, $sPageSlug, $sPageTitle, $sMenuTitle, $sCapability, $bShowInMenu, $nOrder)
    {
        ...
        $_sPageHook = add_submenu_page($sRootPageSlug, $sPageTitle, $sMenuTitle, $sCapability, $sPageSlug, [$this->oFactory, '_replyToRenderPage']);
    ...
    private function _setPageHooks($sPageHook, $sPageSlug)
    {
        ...
        add_action('current_screen', [$this->oFactory, 'load_pre_'.$sPageSlug], 20);
        add_action('load_'.$sPageSlug, [$this->oFactory, '_replyToFinalizeInPageTabs'], 9999);
        add_action('load_after_'.$sPageSlug, [$this->oFactory, '_replyToEnqueuePageAssets']);
        add_action('load_after_'.$sPageSlug, [$this->oFactory, '_replyToEnablePageMetaBoxes']);
    ...
    {
        $_nNewIndex = $this->getUnusedNumericIndex($this->getElementAsArray($GLOBALS, ['submenu', $sMenuSlug]), $nOrder, 5);
    ...
    {
        foreach ($this->getElementAsArray($GLOBALS, ['submenu', $sMenuSlug]) as $_iIndex => $_aSubMenu) {
            if (!isset($_aSubMenu[3])) {
                continue;
            }
            $_aA = [$_aSubMenu[0], $_aSubMenu[3], $_aSubMenu[2]];
            $_aB = [$sMenuTitle, $sPageTitle, $sPageSlug];
            if ($_aA !== $_aB) {
                continue;
            }
    ...
    {
        $_aSubMenuItem = $this->getElementAsArray($GLOBALS, ['submenu', $sMenuSlug, $_iIndex]);
    ...
    private function _addLinkSubmenuItem($sMenuSlug, $sTitle, $sCapability, $sHref, $bShowInMenu, $nOrder)
    {
        ...
        $_aSubMenuItems = $this->getElementAsArray($GLOBALS, ['submenu', $sMenuSlug]);
        $_nIndex = $this->getUnusedNumericIndex($_aSubMenuItems, $nOrder, 5);
        $_aSubMenuItems[$_nIndex] = [$sTitle, $sCapability, $sHref];
