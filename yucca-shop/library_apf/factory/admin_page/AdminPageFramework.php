...
abstract class yucca_shopAdminPageFramework_Router extends yucca_shopAdminPageFramework_Factory
{
    public function __construct($sOptionKey = null, $sCallerPath = null, $sCapability = 'manage_options', $sTextDomain = 'yucca-shop')
    {
        $this->oProp = isset($this->oProp) ? $this->oProp : new yucca_shopAdminPageFramework_Property_admin_page($this, $sCallerPath, get_class($this), $sOptionKey, $sCapability, $sTextDomain);
        ...
        add_action('wp_loaded', [$this, '_replyToDetermineToLoad']);
    }

    protected function _getLinkObject()
    {
        return new yucca_shopAdminPageFramework_Link_admin_page($this->oProp, $this->oMsg);
    }

    protected function _getPageLoadObject()
    {
        return new yucca_shopAdminPageFramework_PageLoadInfo_admin_page($this->oProp, $this->oMsg);
    }
    ...
    public function __call($sMethodName, $aArgs = null)
    {
        ...
        $_aKnownMethodPrefixes = ['section_pre_', 'field_pre_', 'load_pre_'];
    ...
    private function _getCallbackName($sMethodName, $sPageSlug, array $aKnownMethodPrefixes = [])
    {
    ...
    protected function _doPageLoadCall($sMethodName, $sPageSlug, $sTabSlug, $oScreen)
    {
        ...
        $this->oUtil->addAndDoActions($this, ["load_{$this->oProp->sClassName}", "load_{$sPageSlug}"], $this);
        $this->oUtil->addAndDoActions($this, ["load_{$sPageSlug}_".$this->oProp->getCurrentTabSlug($sPageSlug)], $this);
        $this->oUtil->addAndDoActions($this, ["load_after_{$this->oProp->sClassName}", "load_after_{$sPageSlug}"], $this);
    }
...
abstract class yucca_shopAdminPageFramework_Model_Form extends yucca_shopAdminPageFramework_Router
{
    public $aFieldErrors;
    protected static $_sStructureType = 'admin_page';
    ...
    public function __construct($sOptionKey = null, $sCallerPath = null, $sCapability = 'manage_options', $sTextDomain = 'yucca-shop')
    {
        ...
        new yucca_shopAdminPageFramework_Model__FormEmailHandler($this);
    ...
    {
        new yucca_shopAdminPageFramework_Model__FormSubmission($this, $aSavedData, $aArguments, $aSectionsets, $aFieldsets);
    }

    public function _replyToFieldsetResourceRegistration($aFieldset)
    {
        $aFieldset = $aFieldset + ['help' => null, 'title' => null, 'help_aside' => null, 'page_slug' => null, 'tab_slug' => null, 'section_title' => null, 'section_id' => null];
        if (!$aFieldset['help']) {
            return;
        }
        $_sRootSectionID = $this->oUtil->getElement($this->oUtil->getAsArray($aFieldset['section_id']), 0);
        $this->addHelpTab(['page_slug' => $aFieldset['page_slug'], 'page_tab_slug' => $aFieldset['tab_slug'], 'help_tab_title' => $aFieldset['section_title'], 'help_tab_id' => $_sRootSectionID, 'help_tab_content' => "<span class='contextual-help-tab-title'>".$aFieldset['title'].'</span> - '.PHP_EOL.$aFieldset['help'], 'help_tab_sidebar_content' => $aFieldset['help_aside'] ? $aFieldset['help_aside'] : '']);
    }
    ...
    public function _registerHelpPaneItemsOfFormSections($aSectionsets)
    {
        foreach ($aSectionsets as $_aSectionset) {
            $_aSectionset = $_aSectionset + ['help' => null, 'page_slug' => null, 'tab_slug' => null, 'title' => null, 'section_id' => null, 'help' => null, 'help_aside' => null];
            if (empty($_aSectionset['help'])) {
                continue;
            }
            $this->addHelpTab(['page_slug' => $_aSectionset['page_slug'], 'page_tab_slug' => $_aSectionset['tab_slug'], 'help_tab_title' => $_aSectionset['title'], 'help_tab_id' => $_aSectionset['section_id'], 'help_tab_content' => $_aSectionset['help'], 'help_tab_sidebar_content' => $this->getElement($_aSectionset, 'help_aside', '')]);
        }
    }
    ...
    public function _replyToFormatFieldsetDefinition($aFieldset, $aSectionsets)
    {
        ...
        $aFieldset['page_slug'] = $this->oUtil->getElement($aSectionsets, [$_sSectionPath, 'page_slug'], $this->oProp->getCurrentPageSlugIfAdded());
        $aFieldset['tab_slug'] = $this->oUtil->getElement($aSectionsets, [$_sSectionPath, 'tab_slug'], $this->oProp->getCurrentInPageTabSlugIfAdded());
    ...
    public function _replyToFormatSectionsetDefinition($aSectionset)
    {
        ...
        $aSectionset = $aSectionset + ['page_slug' => null, 'tab_slug' => null, 'capability' => null];
    ...
    private function _getSectionCapability($aSectionset)
    {
        ...
        if (0 < $aSectionset['_nested_depth']) {
            ...
            $_sParentCapability = $this->oUtil->getElement($this->oForm->aSectionsets, array_merge($_aSectionPath, ['capability']));
    ...
    private function _getSectionPageSlug($aSectionset)
    {
        ...
        if (0 < $aSectionset['_nested_depth']) {
            ...
            $_sRootSectionPageSlug = $this->oUtil->getElement($this->oForm->aSectionsets, [$_sRootSectionID, 'page_slug']);
    ...
    public function _replyToGetCapabilityForForm($sCapability)
    {
        ...
        $_aCapabilities = array_filter([$_sTabCapability, $_sPageCapability]) + [$this->oProp->sCapability];

        return $_aCapabilities[0];
    }
}
abstract class yucca_shopAdminPageFramework_View_Form extends yucca_shopAdminPageFramework_Model_Form
{
    public function _replyToGetSectionName()
    {
        $_aParams = func_get_args() + [null, null];
        $sNameAttribute = $_aParams[0];
        $aSectionset = $_aParams[1];
        $_aSectionPath = $aSectionset['_section_path_array'];
        $_aDimensionalKeys = [$this->oProp->sOptionKey];
    ...
    public function _replyToGetFieldNameAttribute()
    {
        ...
        $_aDimensionalKeys = [$aFieldset['option_key']];
...
abstract class yucca_shopAdminPageFramework_Controller_Form extends yucca_shopAdminPageFramework_View_Form
{
    ...
    public function addSettingSection($asSection)
    {
        ...
        $aSection = $this->oUtil->uniteArrays($aSection, ['page_slug' => $this->_sTargetPageSlug, 'tab_slug' => $this->_sTargetTabSlug, 'section_tab_slug' => $this->_sTargetSectionTabSlug]);
    ...
    public function getValue()
    {
        ...
        $_aDimensionalKeys = $_aParams + [null, null];
        $_mDefault = null;
        ...
        return yucca_shopAdminPageFramework_WPUtility::getOption($this->oProp->sOptionKey, empty($_aParams) ? null : $_aDimensionalKeys, $_mDefault, $this->getSavedOptions() + $this->oForm->getDefaultFormValues());
    }
...
abstract class yucca_shopAdminPageFramework_Model_Page extends yucca_shopAdminPageFramework_Controller_Form
{
    public function _replyToFinalizeInPageTabs()
    {
        ...
        foreach ($this->oProp->aPages as $_sPageSlug => $_aPage) {
            ...
            $_oFormatter = new yucca_shopAdminPageFramework_Format_InPageTabs($this->oProp->aInPageTabs[$_sPageSlug], $_sPageSlug, $this);
    ...
    {
        return $this->oUtil->getElement($this->oProp->aPages, [$sPageSlug, 'capability']);
    }

    public function _getInPageTabCapability($sTabSlug, $sPageSlug)
    {
        return $this->oUtil->getElement($this->oProp->aInPageTabs, [$sPageSlug, $sTabSlug, 'capability']);
    }
}
abstract class yucca_shopAdminPageFramework_View_Page extends yucca_shopAdminPageFramework_Model_Page
{
    public function _replyToEnablePageMetaBoxes()
    {
        new yucca_shopAdminPageFramework_View__PageMetaboxEnabler($this);
    }

    public function _replyToEnqueuePageAssets()
    {
        new yucca_shopAdminPageFramework_View__Resource($this);
    }
    ...
    {
        $_oPageRenderer = new yucca_shopAdminPageFramework_View__PageRenderer($this, $sPageSlug, $sTabSlug);
        $_oPageRenderer->render();
    }
}
abstract class yucca_shopAdminPageFramework_Controller_Page extends yucca_shopAdminPageFramework_View_Page
{
    ...
    public function addInPageTab($asInPageTab)
    {
        ...
        $aInPageTab = $asInPageTab + ['page_slug' => $__sTargetPageSlug, 'tab_slug' => null, 'order' => null];
        $__sTargetPageSlug = $aInPageTab['page_slug'];
        if (!isset($aInPageTab['page_slug'], $aInPageTab['tab_slug'])) {
            return;
        }
        $_aElements = $this->oUtil->getElement($this->oProp->aInPageTabs, $aInPageTab['page_slug'], []);
        $_iCountElement = count($_aElements);
        $aInPageTab = ['page_slug' => $this->oUtil->sanitizeSlug($aInPageTab['page_slug']), 'tab_slug' => $this->oUtil->sanitizeSlug($aInPageTab['tab_slug']), 'order' => $this->oUtil->getAOrB(is_numeric($aInPageTab['order']), $aInPageTab['order'], $_iCountElement + 10)] + $aInPageTab;
...
abstract class yucca_shopAdminPageFramework_Model_Menu extends yucca_shopAdminPageFramework_Controller_Page
{
    public function __construct($sOptionKey = null, $sCallerPath = null, $sCapability = 'manage_options', $sTextDomain = 'yucca-shop')
    {
        parent::__construct($sOptionKey, $sCallerPath, $sCapability, $sTextDomain);
        ...
        new yucca_shopAdminPageFramework_Model_Menu__RegisterMenu($this);
    }
}
abstract class yucca_shopAdminPageFramework_View_Menu extends yucca_shopAdminPageFramework_Model_Menu
{
}
abstract class yucca_shopAdminPageFramework_Controller_Menu extends yucca_shopAdminPageFramework_View_Menu
{
    protected $_aBuiltInRootMenuSlugs = ['dashboard' => 'index.php', 'posts' => 'edit.php', 'media' => 'upload.php', 'links' => 'link-manager.php', 'pages' => 'edit.php?post_type=page', 'comments' => 'edit-comments.php', 'appearance' => 'themes.php', 'plugins' => 'plugins.php', 'users' => 'users.php', 'tools' => 'tools.php', 'settings' => 'options-general.php', 'network admin' => 'network_admin_menu'];
    ...
    public function setRootMenuPage($sRootMenuLabel, $sIcon16x16 = null, $iMenuPosition = null)
    {
        ...
        $this->oProp->aRootMenu = ['sTitle' => $sRootMenuLabel, 'sPageSlug' => $_sSlug ? $_sSlug : str_replace('\\', '_', $this->oProp->sClassName), 'sIcon16x16' => $this->oUtil->getResolvedSRC($sIcon16x16), 'iPosition' => $iMenuPosition, 'fCreateRoot' => empty($_sSlug)];
    }
    ...
    public function addSubMenuLink(array $aSubMenuLink)
    {
        ...
        $_oFormatter = new yucca_shopAdminPageFramework_Format_SubMenuLink($aSubMenuLink, $this, count($this->oProp->aPages) + 1);
    ...
    public function addSubMenuPage(array $aSubMenuPage)
    {
        ...
        $_oFormatter = new yucca_shopAdminPageFramework_Format_SubMenuPage($aSubMenuPage, $this, count($this->oProp->aPages) + 1);
        $_aSubMenuPage = $_oFormatter->get();
        $this->oProp->aPages[$_aSubMenuPage['page_slug']] = $_aSubMenuPage;
    }
}
abstract class yucca_shopAdminPageFramework_Model extends yucca_shopAdminPageFramework_Controller_Menu
{
}
abstract class yucca_shopAdminPageFramework_View extends yucca_shopAdminPageFramework_Model
{
...
abstract class yucca_shopAdminPageFramework_Controller extends yucca_shopAdminPageFramework_View
{
    ...
    public function setAdminNotice($sMessage, $sClassSelector = 'error', $sID = '')
    {
        ...
        $this->oProp->aAdminNotices[$sID] = ['sMessage' => $sMessage, 'aAttributes' => ['id' => $sID, 'class' => $sClassSelector]];
        new yucca_shopAdminPageFramework_AdminNotice($this->oProp->aAdminNotices[$sID]['sMessage'], $this->oProp->aAdminNotices[$sID]['aAttributes'], ['should_show' => [$this, '_isInThePage']]);
    }
    ...
    {
        return yucca_shopAdminPageFramework_WPUtility::getOption($sOptionKey, $asKey, $vDefault);
    }
}
abstract class yucca_shopAdminPageFramework extends yucca_shopAdminPageFramework_Controller
{
    public function __construct($isOptionKey = null, $sCallerPath = null, $sCapability = 'manage_options', $sTextDomain = 'yucca-shop')
    {
        if (!$this->_isInstantiatable()) {
            return;
        }
        parent::__construct($isOptionKey, $sCallerPath ? trim($sCallerPath) : $sCallerPath = (is_admin() && (isset($GLOBALS['pagenow']) && in_array($GLOBALS['pagenow'], ['plugins.php']) || isset($_GET['page'])) ? yucca_shopAdminPageFramework_Utility::getCallerScriptPath(__FILE__) : null), $sCapability, $sTextDomain);
    }
}
