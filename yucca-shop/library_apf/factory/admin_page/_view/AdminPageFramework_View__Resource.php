...
class yucca_shopAdminPageFramework_View__Resource extends yucca_shopAdminPageFramework_FrameworkUtility
{
    public $oFactory;
    ...
    {
        add_action("style_{$this->sCurrentPageSlug}", [$this, '_replyToAddInlineCSSRules']);
        if ($this->sCurrentTabSlug) {
            add_action("style_{$this->sCurrentPageSlug}_{$this->sCurrentTabSlug}", [$this, '_replyToAddInlineCSSRules']);
        }
        add_action("script_{$this->sCurrentPageSlug}", [$this, '_replyToAddInlineScripts']);
        if ($this->sCurrentTabSlug) {
            add_action("script_{$this->sCurrentPageSlug}_{$this->sCurrentTabSlug}", [$this, '_replyToAddInlineScripts']);
        }
    }
    ...
    {
        $_aPageStyles = $this->getElementAsArray($oFactory->oProp->aPages, [$this->sCurrentPageSlug, 'style']);
        $this->_enqueuePageAssets($_aPageStyles, 'style');
        $_aPageScripts = $this->getElementAsArray($oFactory->oProp->aPages, [$this->sCurrentPageSlug, 'script']);
        $this->_enqueuePageAssets($_aPageScripts, 'script');
        if (!$this->sCurrentTabSlug) {
            return;
        }
        $_aInPageTabStyles = $this->getElementAsArray($oFactory->oProp->aInPageTabs, [$this->sCurrentPageSlug, $this->sCurrentTabSlug, 'style']);
        $this->_enqueuePageAssets($_aInPageTabStyles, 'style');
        $_aInPageTabScripts = $this->getElementAsArray($oFactory->oProp->aInPageTabs, [$this->sCurrentPageSlug, $this->sCurrentTabSlug, 'script']);
    ...
    {
        $_oFormatter = new yucca_shopAdminPageFramework_Format_PageResource_Style($asPageStyle);
    ...
    {
        $_oFormatter = new yucca_shopAdminPageFramework_Format_PageResource_Script($asPageScript);
