...
class yucca_shopAdminPageFramework_View__PageMetaboxEnabler extends yucca_shopAdminPageFramework_FrameworkUtility
{
    public $oFactory;
    ...
    public function __construct($oFactory)
    {
        ...
        add_action('admin_head', [$this, '_replyToEnableMetaBox']);
    }
    ...
    public function _replyToEnableMetaBox()
    {
        ...
        add_screen_option('layout_columns', ['max' => $_iColumns, 'default' => $_iColumns]);
        wp_nonce_field('meta-box-order', 'meta-box-order-nonce', false);
        wp_nonce_field('closedpostboxes', 'closedpostboxesnonce', false);
        if (isset($GLOBALS['page_hook'])) {
            add_action("admin_footer-{$GLOBALS['page_hook']}", [$this, '_replyToAddMetaboxScript']);
        }
    }

    private function _isMetaBoxAdded()
    {
        $_aPageMetaBoxClasses = $this->getElementAsArray($GLOBALS, ['ayucca_shopAdminPageFramework', 'aMetaBoxForPagesClasses']);
    ...
    {
        $_bLoaded = $this->getElement($GLOBALS, ['ayucca_shopAdminPageFramework', 'bAddedMetaBoxScript'], false);
        if ($_bLoaded) {
            return;
        }
        $GLOBALS['ayucca_shopAdminPageFramework']['bAddedMetaBoxScript'] = true;
        $_sScript = <<<'JAVASCRIPTS'
jQuery( document).ready( function(){ 
    postboxes.add_postbox_toggles( pagenow ); 
});
JAVASCRIPTS;
        echo '<script class="yucca-shop-insert-metabox-script">'.'/* <![CDATA[ */'.$_sScript.'/* ]]> */'.'</script>';
    }
}
