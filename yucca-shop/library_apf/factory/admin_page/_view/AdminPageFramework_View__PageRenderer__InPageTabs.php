...
class yucca_shopAdminPageFramework_View__PageRenderer__InPageTabs extends yucca_shopAdminPageFramework_FrameworkUtility
{
    public $oFactory;
    public $sPageSlug;
    public $sTag = 'h3';
    ...
    public function get()
    {
        $_aInPageTabs = $this->getElement($this->oFactory->oProp->aInPageTabs, $this->sPageSlug, []);
    ...
    {
        $_oTabBar = new yucca_shopAdminPageFramework_TabNavigationBar($aTabs, $sActiveTab, $sTag, ['class' => 'in-page-tab'], ['format' => [$this, '_replyToFormatNavigationTabItem_InPageTab'], 'arguments' => ['page_slug' => $sCurrentPageSlug]]);
        $_sTabBar = $_oTabBar->get();

        return $_sTabBar ? "<div class='yucca-shop-in-page-tab'>".$_sTabBar.'</div>' : '';
    }

    public function _replyToFormatNavigationTabItem_InPageTab(array $aTab, array $aStructure, array $aTabs, array $aArguments = [])
    {
        $_oFormatter = new yucca_shopAdminPageFramework_Format_NavigationTab_InPageTab($aTab, $aStructure, $aTabs, $aArguments, $this->oFactory);
    ...
    {
        $_sParentTabSlug = $this->getElement($this->oFactory->oProp->aInPageTabs, [$sPageSlug, $sTabSlug, 'parent_tab_slug'], $sTabSlug);
