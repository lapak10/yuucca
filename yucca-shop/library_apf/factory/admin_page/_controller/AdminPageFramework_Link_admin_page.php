...
class yucca_shopAdminPageFramework_Link_admin_page extends yucca_shopAdminPageFramework_Link_Base
{
    public $oProp;
    ...
    public function _addLinkToPluginDescription($asLinks)
    {
        ...
        add_filter('plugin_row_meta', [$this, '_replyToAddLinkToPluginDescription'], 10, 2);
    }
    ...
    private function _addFilterHook_PluginTitleActionLink()
    {
        ...
        add_filter($this->_sFilterSuffix_PluginActionLinks.$_sPluginBaseName, [$this, '_replyToAddPluginActionLinks']);
    }
    ...
    public function _replyToAddInfoInFooterLeft($sLinkHTML = '')
    {
        ...
        return $this->addAndApplyFilters($this->oProp->oCaller, [$this->getAOrB($_sTabSlug, 'footer_left_'.$_sPageSlug.'_'.$_sTabSlug, null), 'footer_left_'.$_sPageSlug, 'footer_left_'.$this->oProp->sClassName], $sLinkHTML);
    }
    ...
    public function _replyToAddInfoInFooterRight($sLinkHTML = '')
    {
        ...
        return $this->addAndApplyFilters($this->oProp->oCaller, [$this->getAOrB($_sTabSlug, 'footer_right_'.$_sPageSlug.'_'.$_sTabSlug, null), 'footer_right_'.$_sPageSlug, 'footer_right_'.$this->oProp->sClassName], $this->oProp->aFooterInfo['sRight']);
    }
    ...
    public function _replyToAddSettingsLinkInPluginListingPage($aLinks)
    {
        ...
        $_sLinkURL = preg_match('/^.+\.php/', $this->oProp->aRootMenu['sPageSlug']) ? add_query_arg(['page' => $this->oProp->sDefaultPageSlug], admin_url($this->oProp->aRootMenu['sPageSlug'])) : "admin.php?page={$this->oProp->sDefaultPageSlug}";
        array_unshift($aLinks, '<a '.$this->getAttributes(['href' => esc_url($_sLinkURL), 'class' => 'apf-plugin-title-action-link apf-post-type']).'>'.$this->oProp->sLabelPluginSettingsLink.'</a>');
