...
class yucca_shopAdminPageFramework_Resource_admin_page extends yucca_shopAdminPageFramework_Resource_Base
{
    ...
    public function _enqueueStyles($aSRCs, $sPageSlug = '', $sTabSlug = '', $aCustomArgs = [])
    {
        $_aHandleIDs = [];
    ...
    public function _enqueueStyle($sSRC, $sPageSlug = '', $sTabSlug = '', $aCustomArgs = [])
    {
        return $this->_enqueueResourceByType($sSRC, $sPageSlug, $sTabSlug, $aCustomArgs, 'style');
    }

    public function _enqueueScripts($aSRCs, $sPageSlug = '', $sTabSlug = '', $aCustomArgs = [])
    {
        $_aHandleIDs = [];
        foreach ((array) $aSRCs as $_sSRC) {
    ...
    public function _enqueueScript($sSRC, $sPageSlug = '', $sTabSlug = '', $aCustomArgs = [])
    {
        return $this->_enqueueResourceByType($sSRC, $sPageSlug, $sTabSlug, $aCustomArgs, 'script');
    }

    private function _enqueueResourceByType($sSRC, $sPageSlug = '', $sTabSlug = '', $aCustomArgs = [], $sType = 'style')
    {
        $sSRC = trim($sSRC);
        if (empty($sSRC)) {
            return '';
        ...
        $this->oProp->{$_sContainerPropertyName}[$_sSRCHash] = array_filter($this->getAsArray($aCustomArgs), [$this, 'isNotNull']) + ['sPageSlug' => $sPageSlug, 'sTabSlug' => $sTabSlug, 'sSRC' => $sSRC, 'sType' => $sType, 'handle_id' => $sType.'_'.$this->oProp->sClassName.'_'.(++$this->oProp->{$_sEnqueuedIndexPropertyName})] + self::$_aStructure_EnqueuingResources;
