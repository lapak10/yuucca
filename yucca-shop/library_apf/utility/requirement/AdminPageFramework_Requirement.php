...
class yucca_shopAdminPageFramework_Requirement
{
    private $_aRequirements = [];
    public $aWarnings = [];
    private $_aDefaultRequirements = ['php' => ['version' => '5.2.4', 'error' => 'The plugin requires the PHP version %1$s or higher.'], 'wordpress' => ['version' => '3.3', 'error' => 'The plugin requires the WordPress version %1$s or higher.'], 'mysql' => ['version' => '5.0', 'error' => 'The plugin requires the MySQL version %1$s or higher.'], 'functions' => [], 'classes' => [], 'constants' => [], 'files' => []];

    public function __construct(array $aRequirements = [], $sScriptName = '')
    {
        $aRequirements = $aRequirements + $this->_aDefaultRequirements;
        $aRequirements = array_filter($aRequirements, 'is_array');
        foreach (['php', 'mysql', 'wordpress'] as $_iIndex => $_sName) {
    ...
    public function check()
    {
        ...
        $this->_aRequirements = $this->_aRequirements + ['functions' => [], 'classes' => [], 'constants' => [], 'files' => []];
    ...
    {
        return empty($aClasses) ? [] : $this->_getWarningsByFunctionName('class_exists', $aClasses);
    }

    private function _checkFunctions($aFunctions)
    {
        return empty($aFunctions) ? [] : $this->_getWarningsByFunctionName('function_exists', $aFunctions);
    }

    private function _checkConstants($aConstants)
    {
        return empty($aConstants) ? [] : $this->_getWarningsByFunctionName('defined', $aConstants);
    }

    private function _checkFiles($aFilePaths)
    {
        return empty($aFilePaths) ? [] : $this->_getWarningsByFunctionName('file_exists', $aFilePaths);
    }
    ...
    private function _getWarningsByFunctionName($sFuncName, $aSubjects)
    {
        ...
        foreach ($aSubjects as $_sSubject => $_sWarning) {
            if (!call_user_func_array($sFuncName, [$_sSubject])) {
    ...
    {
        add_action('admin_notices', [$this, '_replyToPrintAdminNotices']);
    }
    ...
    public function deactivatePlugin($sPluginFilePath, $sMessage = '', $bIsOnActivation = false)
    {
        ...
        if ($bIsOnActivation) {
            $_sPluginListingPage = add_query_arg([], $GLOBALS['pagenow']);
