...
class yucca_shopAdminPageFramework_Zip
{
    public $sSource;
    public $sDestination;
    public $aCallbacks = ['file_name' => null, 'file_contents' => null, 'directory_name' => null];
    public $aOptions = ['include_directory' => false, 'additional_source_directories' => []];

    public function __construct($sSource, $sDestination, $abOptions = false, array $aCallbacks = [])
    {
    ...
    {
        $_aOptions = is_array($abOptions) ? $abOptions : ['include_directory' => $abOptions];
    ...
    public function compress()
    {
        ...
        $_aMethods = ['unknown' => '_replyToReturnFalse', 'directory' => '_replyToCompressDirectory', 'file' => '_replyToCompressFile'];
        $_sMethodName = $_aMethods[$this->_getSourceType($this->sSource)];

        return call_user_func_array([$this, $_sMethodName], [$_oZip, $this->sSource, $this->aCallbacks, $this->aOptions['include_directory'], $this->aOptions['additional_source_directories']]);
    }
    ...
    public function _replyToCompressDirectory(ZipArchive $oZip, $sSourceDirPath, array $aCallbacks = [], $bIncludeDir = false, array $aAdditionalSourceDirs = [])
    {
    ...
    private function _addArchiveItems($oZip, $aSourceDirPaths, $aCallbacks, $sRootDirName = '')
    {
        ...
        foreach ($aSourceDirPaths as $_isIndexOrRelativeDirPath => $_sSourceDirPath) {
            ...
            $_sInsideDirPrefix = is_int($_isIndexOrRelativeDirPath) ? '' : $_isIndexOrRelativeDirPath;
    ...
    private function _addArchiveItem(ZipArchive $oZip, $sSource, $_sIterationItem, array $aCallbacks, $sInsidePathPrefix = '')
    {
        ...
        if (in_array(substr($_sIterationItem, strrpos($_sIterationItem, '/') + 1), ['.', '..'])) {
            return;
        }
    ...
    private function _addFromString(ZipArchive $oZip, $sInsidePath, $sSourceContents = '', array $aCallbacks = [])
    {
        $sInsidePath = $this->_getFilteredArchivePath($sInsidePath, $aCallbacks['file_name']);
        if (!strlen($sInsidePath)) {
            return;
        }
        $oZip->addFromString($sInsidePath, is_callable($aCallbacks['file_contents']) ? call_user_func_array($aCallbacks['file_contents'], [$sSourceContents, $sInsidePath]) : $sSourceContents);
    }

    private function _getFilteredArchivePath($sArchivePath, $oCallable)
    {
        return is_callable($oCallable) ? call_user_func_array($oCallable, [$sArchivePath]) : $sArchivePath;
    }
}
