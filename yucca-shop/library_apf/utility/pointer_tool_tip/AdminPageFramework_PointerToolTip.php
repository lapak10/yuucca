...
class yucca_shopAdminPageFramework_PointerToolTip extends yucca_shopAdminPageFramework_FrameworkUtility
{
    private static $_bResourceLoaded = false;
    private static $aPointers = [];
    public $sPointerID;
    public $aPointerData;
    public $aScreenIDs = [];
    ...
    private function _setHooks($aScreenIDs)
    {
        foreach ($aScreenIDs as $_sScreenID) {
            ...
            add_filter(get_class($this).'-'.$_sScreenID, [$this, '_replyToSetPointer']);
        }
        if (!$this->_hasBeenCalled()) {
            return;
        }
        add_action('admin_enqueue_scripts', [$this, '_replyToLoadPointers'], 1000);
    }
    ...
    {
        return [$this->sPointerID => $this->aPointerData] + $aPointers;
    }
    ...
    private function _getPointers()
    {
        ...
        if (in_array($_sScreenID, $this->aScreenIDs)) {
            return apply_filters(get_class($this).'-'.$_sScreenID, []);
        }
        if (isset($_GET['page'])) {
            return apply_filters(get_class($this).'-'.$_GET['page'], []);
        }
    ...
    private function _getValidPointers($_aPointers)
    {
        ...
        foreach ($_aPointers as $_iPointerID => $_aPointer) {
            $_aPointer = $_aPointer + ['target' => null, 'options' => null, 'pointer_id' => null];
    ...
    private function _enqueueScripts()
    {
        ...
        add_action('admin_print_footer_scripts', [$this, '_replyToInsertInlineScript']);
    }

    public function _replyToInsertInlineScript()
    {
        echo "<script type='text/javascript' class='yucca-shop-pointer-tool-tip'>".'/* <![CDATA[ */'.$this->_getInlineScript(self::$aPointers).'/* ]]> */'.'</script>';
    }

    public function _getInlineScript($aPointers = [])
    {
