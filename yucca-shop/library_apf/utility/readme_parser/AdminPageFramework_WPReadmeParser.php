...
class yucca_shopAdminPageFramework_WPReadmeParser
{
    private static $_aStructure_Callbacks = ['code_block' => null, 'content_before_parsing' => null];
    private static $_aStructure_Replacements = [];
    public $sText = '';
    protected $_aSections = [];
    public $aReplacements = [];
    public $aCallbacks = [];

    public function __construct($sFilePathOrContent = '', array $aReplacements = [], array $aCallbacks = [])
    {
        ...
        $this->_aSections = $this->sText ? $this->_getSplitContentsBySection($this->sText) : [];
    }
    ...
    {
        $_sContent = is_callable($this->aCallbacks['content_before_parsing']) ? call_user_func_array($this->aCallbacks['content_before_parsing'], [$sContent]) : $sContent;
        $_sContent = preg_replace('/`(.*?)`/', '<code>\\1</code>', $_sContent);
        $_sContent = preg_replace_callback('/`(.*?)`/ms', [$this, '_replyToReplaceCodeBlocks'], $_sContent);
        ...
        $_oParsedown = new yucca_shopAdminPageFramework_Parsedown();
    ...
    public function _replyToReplaceCodeBlocks($aMatches)
    {
        ...
        return is_callable($this->aCallbacks['code_block']) ? call_user_func_array($this->aCallbacks['code_block'], [$_sModified, $_sIntact]) : $_sModified;
    }
